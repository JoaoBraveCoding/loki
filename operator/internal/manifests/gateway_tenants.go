...
func configureGatewayDeploymentForMode(d *appsv1.Deployment, tenants *lokiv1.TenantsSpec, fg configv1.FeatureGates, minTLSVersion string, ciphers string, adminGroups []string, adminServiceAccounts []string) error {
	switch tenants.Mode {
	case lokiv1.Static, lokiv1.Dynamic:
		if tenants != nil {
	...
	case lokiv1.OpenshiftLogging, lokiv1.OpenshiftNetwork:
		...
		return openshift.ConfigureGatewayDeployment(d, tenants.Mode, tlsSecretVolume, tlsDir, minTLSVersion, ciphers, fg.HTTPEncryption, adminGroups, adminServiceAccounts)
	}
